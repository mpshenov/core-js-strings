...
function removeFirstOccurrences(str, value) {
  const removedIndex = str.indexOf(value);
  if (removedIndex === -1) {
    return str;
  }
  return str.slice(0, removedIndex) + str.slice(removedIndex + value.length);
}
...
function removeLastOccurrences(str, value) {
  const removedIndex = str.lastIndexOf(value);
  if (removedIndex === -1) {
    return str;
  }
  return str.slice(0, removedIndex) + str.slice(removedIndex + value.length);
...
function reverseWords(str) {
  const variable1 = str.split(' ');
  const reversedWords = [];
  variable1.forEach((element) => {
    const variable2 = element.split('').reverse().join('');
    reversedWords.push(variable2);
  });
...
function getStringFromTemplate(firstName, lastName) {
  const greetings = 'Hello, '.concat(firstName, ' ', lastName, '!');
  return greetings;
}
